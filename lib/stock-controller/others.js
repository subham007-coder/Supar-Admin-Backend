  ...
  try {
    for (const p of cart) {
      ...
      if (product.isCombination && p.selectedLength && p.selectedCurl) {
        ...
        // Handle simple products (no variants) - use totalStock
        console.log("📦 [STOCK DEBUG] Updating simple product stock using totalStock");
        
        // Check if enough total stock is available
        if (product.totalStock < p.quantity) {
          console.error("❌ [STOCK DEBUG] Insufficient total stock:", {
            requested: p.quantity,
            available: product.totalStock
          });
          throw new Error(`Insufficient stock. Available: ${product.totalStock}, Requested: ${p.quantity}`);
        }
        ...
        console.log("✅ [STOCK DEBUG] Simple product stock updated successfully using totalStock");
      }
    }

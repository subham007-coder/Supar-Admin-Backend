...
const sendEmail = (body, res, message) => {
  ...
  if (process.env.BREVO_API_KEY) {
    ...
    // Prefer 465 (SSL) to avoid provider timeouts on 587 in some environments
    const brevoHost = process.env.BREVO_SMTP_HOST || 'smtp-relay.brevo.com';
    const brevoPort = Number(process.env.BREVO_SMTP_PORT || 465);
    const brevoSecure = brevoPort === 465; // SSL on 465, STARTTLS on 587
    transporterOptions = {
      host: brevoHost,
      port: brevoPort,
      secure: brevoSecure,
      auth: {
        user: 'apikey', // per Brevo docs
